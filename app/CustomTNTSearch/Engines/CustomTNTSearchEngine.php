...
namespace App\CustomTNTSearch\Engines;
...
class CustomTNTSearchEngine extends Engine
{
    /**
     * @var TNTSearch
     */
    ...
    {
        $indexNames = $models->first()->customSearchableAs();
        
        for ($i = 0; $i < count($indexNames); $i++) {

            $this->initIndex($models->first(), $indexNames[$i]);
            $this->tnt->selectIndex("{$indexNames[$i]}.index");
            $index = $this->tnt->getIndex();
            $index->setPrimaryKey($models->first()->getKeyName());
    
            $index->indexBeginTransaction();
            $models->each(function ($model) use ($index, $i) {
                $array = $model->customToSearchableArray($i);
    ...
    public function delete($models)
    {
        ...
        foreach ($indexNames as $indexName) {
            ...
            $this->initIndex($models->first(), $indexName);

            $models->each(function ($model) {
                $this->tnt->selectIndex("{$indexName}.index");
                $index = $this->tnt->getIndex();
    ...
    public function search(Builder $builder)
    {
        try {
            ...
            foreach ($indexNames as $indexName) {
                $this->initIndex($model, $indexName);
            }
        }
    }
    ...
    public function initIndex($model, $indexName)
    {
